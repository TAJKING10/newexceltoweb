...
try {
    ...
    if (Test-Path "payslip-web") {
        ...
        if ($IncludeBuild -and (Test-Path "payslip-web\build")) {
            ...
            Write-Host "  ⚠️  Production build not included (use -IncludeBuild to include)" -ForegroundColor Yellow
        }
    }
    ...
    if (Test-Path "supabase") {
        ...
        if ($Verbose) { Write-Host "  ✓ Supabase configuration" -ForegroundColor Green }
    }
...
"@
    ...
    $SetupScript | Out-File -FilePath (Join-Path $DeploymentDir "scripts\quick-setup.sh") -Encoding UTF8
...
"@
    ...
    $WindowsScript | Out-File -FilePath (Join-Path $DeploymentDir "scripts\setup-windows.bat") -Encoding UTF8
...
Write-Host "Deployment package ready for OVHcloud!" -ForegroundColor Green